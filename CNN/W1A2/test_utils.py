...
def summary(model):
    model.compile(optimizer='adam',
                  ...
                  metrics=['accuracy'])
    result = []
    input_shape = model.input_shape  # Get the model's input shape
    for layer in model.layers:
        # Use layer.output_shape if available, otherwise use compute_output_shape
        ...
        result.append(descriptors)
        input_shape = output_shape  # Update input shape for the next layer
