...
def identity_block_test(target):
    ...
    tf.random.set_seed(2)
    A4 = target(X,
                f = 2,  # change from 3 to 2
                filters = [3, 3, 3],
                initializer=lambda seed=7:constant(value=1), training=True)
    A4np = A4.numpy()
    ...
    assert np.allclose(resume, 
                       ...
                       atol = 1e-4 ), "Wrong values with training=Truee"
...
def convolutional_block_test(target):
    ...
    convolutional_block_output3 = [[[[0.,         2.78575   , 0.,         0.        , 1.6972313 , 2.8261633 ,],
                                     [0.,         1.545085  , 0.,         0.        , 2.1773627 , 1.3912168 ,]],
                                    [[0.,         1.9407238 , 0.,         0.        , 1.477576  , 1.9165102 ,],
                                     [0.,         0.        , 0.,         0.99922   , 1.1174145 , 0.        ,]],]
                                   [[[0.,         0.40810618, 0.,         0.61065334, 0.62125754, 0.43865162,],
                                     [0.,         0.40810618, 0.,         0.61065334, 0.62125754, 0.43865162,]],
                                    [[0.,         0.40810618, 0.,         0.61065334, 0.62125754, 0.43865162,],
                                     [0.,         0.40810618, 0.,         0.61065334, 0.62125754, 0.43865162,]]]]
    ...
    print("C:", C)
    assert np.allclose(C.numpy(), convolutional_block_output3), "Wrong values when training=True."
